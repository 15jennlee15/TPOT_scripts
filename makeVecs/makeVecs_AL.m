DIR.out = '~/Desktop/TPOT/AL/output/';
DIR.in =  '~/Desktop/TPOT/AL/input/';
DIR.vec = '~/Desktop/TPOT/AL/vecs/';
DIR.thisFunk = '~/Desktop/TPOT_scripts/makeVecs/';
...
names = {'Happy' 'Angry' 'Sad' 'CB_Happy' 'CB_Angry' 'CB_Sad' 'Blurry' 'NoResponse'};
nConds = length(names);
    ...
    for r=1:nRuns
        ...
        if ~exist(filenames.out,'file')
            ...
            task.output.raw(:,6) =  [task.output.raw(2:end,3);task.output.raw(end,3)+maxTrialLength] - task.output.raw(:,3);
             ...
             onsets{8} = task.output.raw(isNR,3);
             
             % is this right???
             durations{1} = task.output.raw(isHappy,4);
             durations{2} = task.output.raw(isAngry,4);
             durations{3} = task.output.raw(isSad,4);
             durations{4} = task.output.raw(isBlur,4);
             durations{5} = task.output.raw(isCBHappy,4);
             durations{6} = task.output.raw(isCBAngry,4);
             durations{7} = task.output.raw(isCBSad,4);
             durations{8} = task.output.raw(isNR,6);
