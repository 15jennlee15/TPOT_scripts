...
data=""$outputdir"/"${subid}"/ses-1/dwi/data.nii.gz"
bvecs=""$outputdir"/"${subid}"/ses-1/dwi/bvecs"
bvals=""$outputdir"/"${subid}"/ses-1/dwi/bvals"
b0mask=""$outputdir"/"${subid}"/ses-1/dwi/nodif_brain_mask.nii.gz"
...
bedpostx "$outputdir"/"${subid}"/ses-1/dwi
...
echo "$outputdir"/"${subid}"/ses-1/dwi: MISSING BEDPOSTX INPUT FILES >> $errorlog
fi
