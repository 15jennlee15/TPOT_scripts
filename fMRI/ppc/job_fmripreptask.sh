...
singularity run --bind "${group_dir}":"${group_dir}" $image $bids_dir $derivatives participant --participant_label $subid -w $working_dir -t $task --output-spaces T1w MNI152NLin2009cAsym fsaverage5 fsnative --write-graph --fs-license-file $FS_LICENSE --skip_bids_validation
